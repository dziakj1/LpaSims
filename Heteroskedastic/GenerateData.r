...
for (measurement.quality in c("low","high")) {
  for (class.size.distribution in c("even","uneven")) {
    for (effect.size in c("small","medium","large")) {
      ...
      output.path <- "C:\\Users\\jjd264\\Documents\\Sims-Lpa-Heteroskedastic\\Datasets\\";
      #########################################################
      # Define parameters;
      ...
      if (effect.size == "large") {
        true.distal.mean.by.class <- c(2*.3,0,-2*.3);
        true.distal.sd.by.class <- c(3,1,1);
      }
      if (effect.size == "medium") {
        true.distal.mean.by.class <- c(2*.2,0,-2*.2);
        true.distal.sd.by.class <- c(3,1,1);
      }
      if (effect.size == "small") {
        true.distal.mean.by.class <- c(2*.1,0,-2*.1);
        true.distal.sd.by.class <- c(3,1,1);
      }
