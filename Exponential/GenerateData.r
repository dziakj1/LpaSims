...
for (measurement.quality in c("low","high")) {
  for (class.size.distribution in c("even","uneven")) {
    for (effect.size in c("small","medium","large")) {
      ...
      output.path <- "C:\\Users\\jjd264\\Documents\\Sims-Lpa-Exponential\\Datasets\\";
      #########################################################
      # Define parameters;
      ...
      if (effect.size == "large") {
        true.distal.mean.by.class <- c(1.8,1.40,1); 
      }
      if (effect.size == "medium") {
        true.distal.mean.by.class <- c(1.5,1.25,1); 
      }
      if (effect.size == "small") {
        true.distal.mean.by.class <- c(1.2,1.10,1); 
      }
      true.distal.sd.by.class <- true.distal.mean.by.class;
      true.distal.rate.by.class <- 1/true.distal.mean.by.class;
        ...
        for (this.item in 1:5) {
          for (this.class in 1:3) {
            ...
            distal[which(trueclass==this.class)] <-
                round(rexp(n=which(trueclass==this.class),
                           rate=true.distal.rate.by.class[this.class]),6);
          }
        } 
        #######################################################
